...
test("Write Identifiers", () => {
    ...
    expect(writeIdentifier(identifier)).toEqual("<test>")
})

test("Write String Literals", () => {
    expect(writeValue("test")).toEqual('"test"')
})

test("Write Integer Literals", () => {
    expect(writeValue(LongLiteral.from(5n).unsafeCoerce())).toEqual("5")
})

test("Write Byte Arrays Literals", () => {
    expect(writeValue(new Uint8Array([0,255]))).toEqual("0x00ff")
})

test("Write Set of Statements", () => {
    let statements = [
    ...
    expect(write(statements)).toEqual(expected)
})
